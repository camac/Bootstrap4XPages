package org.openntf.xsp.bootstrap.actions.server;
	...
	private UIDialog findNonNullDialog(String forId) {
		...
		if (StringUtil.isNotEmpty(forId)) {
			...
			if (null != forControl && !(forControl instanceof UIDialog)) {
				String msg = "The Dialog action cannot change the control {0} because it is not a Dialog";						
				msg = StringUtil.format(msg, forId);
				throw new FacesExceptionEx(msg);
			}
			if (forControl == null) {
				String msg = "The Dialog action cannot find a control with the ID {0}.";
				msg = StringUtil.format(msg, forId);
				throw new FacesExceptionEx(msg);
			}
			...
			if (forControl == null) {
				String msg = "The Dialog action cannot find an ancestor Dialog control";						
				throw new FacesExceptionEx(msg);
			}
			return (UIDialog) forControl;
		}
